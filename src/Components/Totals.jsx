...
const Totals = () => {
    ...
    return (
        <nav className="container mx-auto flex flex-col gap-y-12 items-center justify-between p-4">
            <div className="flex justify-between gap-y-2 gap-x-6">
                <div className="flex flex-col items-center justify-between back-ground-image gap-y-2 md:gap-x-6">
                    ...
                    <p className="pt-2 text-xl text-red-600">Cash Average: {" "}
                        {(list?.map((item) => item.amount).reduce((acc, curr) => acc + curr, 0)) / list?.length ?? 0}{" $"}
                    </p>
                </div>
                <div className="flex flex-col items-center justify-between back-ground-image gap-y-2 md:gap-x-6">
                    ...
                    <p className="pt-2 text-xl text-red-600">Credited Accounts: {" "}
                        {list?.filter(acc => acc.amount < 0).length ?? 0}
                    </p>
                </div>
            </div>
